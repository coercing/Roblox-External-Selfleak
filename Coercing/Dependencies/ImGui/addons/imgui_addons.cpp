...
// Apply Style.Alpha to any ImU32 color so all widgets fade with the menu
static ImU32 FA(ImU32 col) {
...
static ImU32 FA(ImColor col) { return FA((ImU32)col); }

// AddRectFilledMultiColorRounded is not available in this ImGui version, approximate with non-rounded version
static void AddRectFilledMultiColorRounded(ImDrawList* dl, const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float /*rounding*/ = 0.0f)
{
...
{
    // Match shadow alpha to the text color alpha so it fades together
    ImVec4 c = ImGui::ColorConvertU32ToFloat4(col);
    ImU32 shadow = IM_COL32(0, 0, 0, (int)(c.w * 255.0f));

    drawList->AddText(ImVec2(pos.x + 1.0f, pos.y + 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x - 1.0f, pos.y - 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x - 1.0f, pos.y + 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x + 1.0f, pos.y - 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x, pos.y + 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x + 1.0f, pos.y), shadow, text);
    drawList->AddText(ImVec2(pos.x, pos.y - 1.0f), shadow, text);
    drawList->AddText(ImVec2(pos.x - 1.0f, pos.y), shadow, text);
...
bool Menu::BeginChild(const char* Label, const ImVec2& SizeArg)
{
    ...
    if (Success)
    {
        ...
        DrawList->AddRectFilled(Pos + ImVec2(1, 1), Pos + Size - ImVec2(1, 1), FA(Menu::Bg), 0.0f);
        ...
        if (HasHeader) Menu::DrawLabelShadow(DrawList, ImVec2(Pos.x + HeaderPadding + Style.FrameBorderSize * 2.0f, Pos.y + HeaderPadding + Style.FrameBorderSize), FA(GetColorU32(ImGuiCol_Text)), StrLabel.c_str());

        if (Style.ChildBorderSize > 0)
        {
            DrawList->AddRect(Pos + ImVec2(1.0f, 1.0f), Pos + Size - ImVec2(1.0f, 1.0f), FA(Menu::DarkAccent), 0);
            DrawList->AddRect(Pos + ImVec2(2.0f, 2.0f), Pos + Size - ImVec2(2.0f, 2.0f), FA(Menu::Outline), 0);
            DrawList->AddLine(Pos + ImVec2(3.0f, 3.0f), ImVec2(Pos.x + Size.x - 3.0f, Pos.y + 3.0f), FA(Menu::Accent), 1.0f);
            DrawList->AddLine(Pos + ImVec2(3.0f, 4.0f), ImVec2(Pos.x + Size.x - 3.0f, Pos.y + 4.0f), FA(Menu::DarkAccent), 1.0f);
        }
...
bool Menu::CheckBox(const char* label, bool* checked)
{
    ...
    if (*checked)
    {
        window->DrawList->AddRectFilled(checkBB.Min, checkBB.Max, FA(boxCol), style.FrameRounding);
        AddRectFilledMultiColorRounded(window->DrawList, checkBB.Min, checkBB.Max, FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,140)), FA(ImColor(0,0,0,140)), style.FrameRounding);
    }
    else
    {
        window->DrawList->AddRectFilled(checkBB.Min, checkBB.Max, FA(boxCol), style.FrameRounding);
        AddRectFilledMultiColorRounded(window->DrawList, checkBB.Min, checkBB.Max, FA(IM_COL32(32,32,32,255)), FA(IM_COL32(32,32,32,255)), FA(IM_COL32(24,24,24,255)), FA(IM_COL32(24,24,24,255)), style.FrameRounding);
    }

    window->DrawList->AddRect(checkBB.Min, checkBB.Max, FA(Menu::Outline), style.FrameRounding);
    window->DrawList->AddRect(checkBB.Min + ImVec2(1,1), checkBB.Max - ImVec2(1,1), FA(activeBox ? Menu::DarkAccent : IM_COL32(44,44,44,255)), style.FrameRounding);

    Menu::DrawLabelShadow(window->DrawList, ImVec2(pos.x + squareSize + style.ItemInnerSpacing.x + 1.0f, pos.y + (squareSize - labelSize.y) * 0.5f), FA(txtCol), label);
...
bool Menu::SliderAlpha(const char* str_id, ImVec4& col)
{
    ...
    AddRectFilledMultiColorRounded(window->DrawList, pos + ImVec2(half_height, 0), pos + ImVec2(width - half_height, height), col_rgb, col_rgb & ~IM_COL32_A_MASK, col_rgb & ~IM_COL32_A_MASK, col_rgb);

    if (style.FrameBorderSize > 0)
        window->DrawList->AddRect(pos, pos + size, GetColorU32(ImGuiCol_Border), style.FrameRounding, style.FrameBorderSize);
...
bool Menu::ColorPalette(const char* str_id, ImVec4& col, const ImVec2& size_arg)
{
    ...
    AddRectFilledMultiColorRounded(window->DrawList,
        bb.Min, bb.Max,
    ...
    if (style.FrameBorderSize > 0)
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), style.FrameRounding, style.FrameBorderSize);
...
bool Menu::ColorButton(const char* desc_id, const ImVec4& col, bool has_alpha, const ImVec2& size_arg)
{
    ...
    if (style.FrameBorderSize > 0)
    {
        window->DrawList->AddRect(pos, pos + size, FA(GetColorU32(ImGuiCol_BorderShadow)), style.FrameRounding, style.FrameBorderSize);
        window->DrawList->AddRect(pos + ImVec2(style.FrameBorderSize,style.FrameBorderSize), pos + size - ImVec2(style.FrameBorderSize,style.FrameBorderSize), FA(IM_COL32(44,44,44,255)), style.FrameRounding, style.FrameBorderSize);
    }
...
bool Menu::ColorEdit4(const char* name, float col[4])
{
    ...
    AddRectFilledMultiColorRounded(window->DrawList, bb_min, bb_max, FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,140)), FA(ImColor(0,0,0,140)), style.FrameRounding);
    window->DrawList->AddRect(bb_min, bb_max, FA(Menu::Outline), 0.0f);
    window->DrawList->AddRect(bb_min + ImVec2(1.0f,1.0f), bb_max - ImVec2(1.0f,1.0f), FA(IM_COL32(44,44,44,255)), 0.0f);
...
bool Menu::SliderScalar(const char* Label, ImGuiDataType DataType, void* PData, const void* PMin, const void* PMax, const char* Format)
{
    ...
    Window->DrawList->AddRectFilledMultiColor(FrameBB.Min, FrameBB.Max, FA(IM_COL32(24,24,24,255)), FA(IM_COL32(24,24,24,255)), FA(IM_COL32(14,14,14,255)), FA(IM_COL32(14,14,14,255)));
    ...
    if (SliderBB.Max.x > SliderBB.Min.x)
        Window->DrawList->AddRectFilled(SliderBB.Min, SliderBB.Max, FA(Menu::Accent), Style.FrameRounding);
    AddRectFilledMultiColorRounded(Window->DrawList, SliderBB.Min, SliderBB.Max, FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,0)), FA(ImColor(0,0,0,140)), FA(ImColor(0,0,0,140)), Style.FrameRounding);

    Window->DrawList->AddRect(FrameBB.Min, FrameBB.Max, FA(IM_COL32(0,0,0,255)), Style.FrameRounding);
    Window->DrawList->AddRect(FrameBB.Min + ImVec2(1.0f,1.0f), FrameBB.Max - ImVec2(1.0f,1.0f), FA(IM_COL32(44,44,44,255)), Style.FrameRounding);
...
bool Menu::SelectableLabel(const char* Label, bool Selected, const ImVec2& SizeArg)
{
    ...
    Window->DrawList->AddText(Pos + ImVec2(1.0f, 0.0f), FA(ItAnim->second.Label), Label);
...
bool Menu::Combo(const char* Label, int* SelectedIndex, std::vector<const char*> Items)
{
    ...
    if (BeginPopupEx(GetID(PopupStrId.c_str()), ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBackground))
    {
        ...
        DL->AddRect(PMin + ImVec2(1,1), PMax - ImVec2(1,1), FA(Menu::Outline));
        DL->AddRect(PMin + ImVec2(2,2), PMax - ImVec2(2,2), FA(IM_COL32(44,44,44,255)));
        ...
        for (int i = 0; i < (int)Items.size(); i++)
        {
            ...
            Menu::DrawLabelShadow(DL, ItemPos + ImVec2(2.0f, 0.0f), FA(CurrentItemCol), Items[i]);
        }
        PopStyleVar();
        EndPopup();
    }
    ...
    Menu::DrawLabelShadow(Window->DrawList, Pos + ImVec2(2.0f, 0.0f), FA(Menu::Text), Label);
    Window->DrawList->AddRectFilledMultiColor(FrameBB.Min, FrameBB.Max, FA(IM_COL32(24,24,24,255)), FA(IM_COL32(24,24,24,255)), FA(IM_COL32(14,14,14,255)), FA(IM_COL32(14,14,14,255)));
    Window->DrawList->AddRect(FrameBB.Min - ImVec2(1,1), FrameBB.Max + ImVec2(1,1), FA(Menu::Outline), Style.FrameRounding);
    Window->DrawList->AddRect(FrameBB.Min, FrameBB.Max, FA(IM_COL32(44,44,44,255)), Style.FrameRounding);

    const char* PreviewItem = (*SelectedIndex < (int)Items.size()) ? Items[*SelectedIndex] : "*unknown*";
    Menu::DrawLabelShadow(Window->DrawList, FrameBB.Min + Style.FramePadding + ImVec2(1.0f, 0.0f), FA(State.Text), PreviewItem);
    ...
    for (int dx = -1; dx <= 1; dx++)
        for (int dy = -1; dy <= 1; dy++)
            if (dx || dy)
                Window->DrawList->AddText(ArrowPos + ImVec2((float)dx, (float)dy), FA(IM_COL32_BLACK), ArrowText);

    Window->DrawList->AddText(ArrowPos, FA(GetColorU32(ImGuiCol_Text)), ArrowText);
...
bool Menu::KeyBind(const char* StrId, ImGuiKey* Key)
{
    ...
    DrawList->AddRectFilledMultiColor(FrameBb.Min, FrameBb.Max, FA(IM_COL32(34,34,34,255)), FA(IM_COL32(34,34,34,255)), FA(IM_COL32(24,24,24,255)), FA(IM_COL32(24,24,24,255)));
    DrawList->AddRect(FrameBb.Min, FrameBb.Max, FA(Menu::Outline), 0.0f);
    DrawList->AddRect(FrameBb.Min + ImVec2(1,1), FrameBb.Max - ImVec2(1,1), FA(IM_COL32(44,44,44,255)), 0.0f);
    ...
    Menu::DrawLabelShadow(DrawList, TextPos, FA(ItAnim->second.Label), KeyName);
